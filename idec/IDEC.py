...
Usage:
    ...
    Weights of Pretrained autoencoder for mnist are in './results/ae_weights/mnist_ae_weights.h5':
        python IDEC.py mnist --ae_weights ./results/ae_weights/mnist_ae_weights.h5
...
class IDEC(object):
    ...
    def fit(self, x, y=None, batch_size=32, maxiter=1e5, tol=1e-3, update_interval=50,
        ...
        for ite in range(int(maxiter)):
            if ite % update_interval == 0:
                ...
                if y is not None:
                    ...
                    self.metrics = dict(iter=ite, acc=acc, nmi=nmi, ari=ari, L=loss[0], Lc=loss[1], Lr=loss[2])
                    logwriter.writerow(self.metrics)
                ...
                # provide feedback for unlabeled data
                if y is None:
                    print('Iter-%d: delta= %.4f' % (ite, delta_label))
...
if __name__ == "__main__":
    ...
    from .datasets import load_mnist, load_reuters, load_usps, load_pendigits, load_bacteria
    data_path = 'data/'
    ...
    y_pred = idec.y_pred
    print(f'acc: {cluster_acc(y, y_pred):.3f}')
